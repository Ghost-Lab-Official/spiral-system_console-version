...
public class spotReportController {
  ...
  /**
    ...
    public static void getTheTotalNumberOfHighlyVisitedSpots() throws Exception {
        ...
        ResultSet rs = stmt.executeQuery("select count(spot_name) from Spot_table  where status=1 AND views>10 ");
        while(rs.next()){
...
//        }
        ...
        while(iterator.hasNext()){
            ...
            System.out.println("\t\t\t " +spot.getSpot_id() + "\t\t\t\t " +spot.getuser_name() +  "\t\t\t\t " +spot.getSpot_description() +  "\t\t\t\t\t " +spot.getLocationName() +  "\t\t\t\t\t " +spot.getCategory_name() +  "\t\t\t\t\t\t\t\t " +spot.getStatus() +  "\t\t\t\t\t\t\t " +spot.getViews() +  "\t\t\t\t\t\t " + spot.getRegistration_date());
        }
        connection.close();
    }
...
//    the List interface, so we can use all the methods of List interface here.
    public static void viewAllInactiveSpots() throws Exception{
        ...
        while(iterate.hasNext()){
            ...
            System.out.println("\t\t\t " +inactspot.getSpot_id() + "\t\t\t\t " +inactspot.getuser_name() +  "\t\t\t\t " +inactspot.getSpot_description() +  "\t\t\t\t\t " +inactspot.getLocationName() +  "\t\t\t\t\t " +inactspot.getCategory_name() +  "\t\t\t\t\t\t\t\t " +inactspot.getStatus() +  "\t\t\t\t\t\t\t " +inactspot.getViews() +  "\t\t\t\t\t\t " + inactspot.getRegistration_date());
        }
        connection.close();
    }
    public static void ViewHighlyVisitedSpots() throws Exception{
        ...
        while(itrt.hasNext()){
            ...
            System.out.println("\t\t\t " +highlyViewed.getSpot_id() + "\t\t\t\t " +highlyViewed.getuser_name() +  "\t\t\t\t " +highlyViewed.getSpot_description() +  "\t\t\t\t\t " +highlyViewed.getLocationName() +  "\t\t\t\t\t " +highlyViewed.getCategory_name() +  "\t\t\t\t\t\t\t\t " +highlyViewed.getStatus() +  "\t\t\t\t\t\t\t " +highlyViewed.getViews() +  "\t\t\t\t\t\t " + highlyViewed.getRegistration_date());
        }
        connection.close();
    }
